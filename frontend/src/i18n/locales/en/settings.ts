export default {
  title: 'System Config',
  subtitle: 'Runtime parameters · Changes take effect immediately, no restart needed',
  reset: 'Reset',
  save: 'Save Config',
  saveSuccess: 'Config saved',
  groups: {
    scan: {
      title: 'Scan & Assets',
      desc: 'Scan batch and asset management parameters',
    },
    session: {
      title: 'Session & Security',
      desc: 'User session timeout settings',
    },
    llm: {
      title: 'LLM Config',
      desc: 'Large language model configuration for topology generation',
    },
    language: {
      title: 'Language',
      desc: 'Interface language settings',
      label: 'Display Language',
    },
  },
  labels: {
    missing_threshold: 'Disappearance Protection Threshold',
    upload_max_size_mb: 'Upload File Limit (MB)',
    asset_no_prefix: 'Asset Number Prefix',
    session_timeout_minutes: 'Session Timeout (min)',
    llm_provider: 'LLM Provider',
    llm_api_key: 'API Key',
    llm_model: 'Model Name',
    llm_base_url: 'API URL',
    llm_ollama_model: 'Local Model Name',
    llm_route_core_to_local: 'Route Core Assets to Local',
    llm_cloud_enabled: 'Allow Cloud LLM',
    feature_cost_accounting_enabled: 'Enable Asset Cost Accounting',
  },
  costCard: {
    title: 'Asset Cost Accounting',
    badgeOn: 'Enabled',
    badgeOff: 'Disabled',
    desc: 'Enable to track and allocate asset costs across departments. Disabling retains existing data but hides cost features from navigation.',
    dep: {
      costOverview: 'Cost Overview',
      deptBilling: 'Department Billing',
      costRates: 'Cost Rates',
      assetCost: 'Asset Cost Panel',
    },
    links: {
      overview: 'Cost Overview',
      billing: 'Department Billing',
      rates: 'Cost Rates',
    },
    confirmTitle: 'Disable Feature',
    confirmOff: 'Existing cost data will be preserved. Are you sure you want to disable Asset Cost Accounting?',
    confirmYes: 'Confirm',
    confirmCancel: 'Cancel',
    enabled: 'Asset Cost Accounting enabled',
    disabled: 'Asset Cost Accounting disabled',
  },
  placeholders: {
    missing_threshold: 'Default 3, mark offline after N consecutive missed scans',
    upload_max_size_mb: 'Default 50',
    asset_no_prefix: 'Default CMDB',
    session_timeout_minutes: 'Default 30',
    llm_api_key: 'Encrypted storage, leave blank to skip update',
    llm_model: 'e.g., deepseek-chat / gpt-4o / claude-3-5-sonnet',
    llm_base_url: 'e.g., https://api.deepseek.com/v1',
    llm_ollama_model: 'Default qwen2.5',
  },
  providers: {
    custom: 'Custom',
    local: 'Local (Ollama)',
  },
  hint: 'API Key and other sensitive fields are stored with <b>Fernet encryption</b>; displayed values are masked.',
  deptManage: {
    title: 'Department Management',
    subtitle: 'Maintain organization departments · Projects can be linked to departments for cost allocation',
    create: 'New Department',
    createTitle: 'New Department',
    editTitle: 'Edit Department',
    colName: 'Department Name',
    colCode: 'Department Code',
    colActions: 'Actions',
    edit: 'Edit',
    delete: 'Delete',
    namePlaceholder: 'e.g., Engineering',
    codePlaceholder: 'e.g., ENG',
    nameCodeRequired: 'Both name and code are required',
    createSuccess: 'Department created',
    updateSuccess: 'Department updated',
    deleteSuccess: 'Department deleted',
    saveError: 'Failed to save department',
    loadError: 'Failed to load departments',
    deleteError: 'Failed to delete department',
    confirmDelete: 'Delete department "{name}"? Linked projects will keep their department value.',
    deleteTitle: 'Confirm Delete',
    noData: 'No departments yet. Create one to get started.',
  },
}
