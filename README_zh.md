...
Z-CMDB 支持三种部署方式：**Windows 双击启动**（零安装、嵌入式 Python）、**Linux 裸机**（venv + systemd）、**Docker**（分层镜像、数据库 bind mount）。所有方式均使用单端口 FastAPI 托管，JWT 密钥自动生成。

| 方式 | 适用场景 | 文档 |
|---|---|---|
| 🪟 Windows 启动包 | 终端用户、离线机器 | [`deploy/DEPLOY.md §1`](deploy/DEPLOY.md#1-windows-双击启动) |
| 🐧 Linux 裸机 | 小型服务器、虚拟机 | [`deploy/DEPLOY.md §2`](deploy/DEPLOY.md#2-linux-裸机部署) |
| 🐳 Docker | CI/CD、容器化环境 | [`deploy/DEPLOY.md §3`](deploy/DEPLOY.md#3-linux-docker-部署) |

> **完整部署文档**：[deploy/DEPLOY.md](deploy/DEPLOY.md)（中文） | [deploy/DEPLOY_EN.md](deploy/DEPLOY_EN.md)（English）

---

## ⚠️ 升级须知

> ### V0.7 — 硬件身份、机房机柜图与安全加固
>
> V0.7 在安全加固之外，新增**硬件资产唯一身份**与**机房机柜视图**：
>
> - **硬件唯一身份**：序列号 SN、品牌厂商、具体型号、资产标签/条码
> - **结构化机柜位置**：机房 / 机柜 / 起始 U 位 / 占 U 数（原“物理位置”文本保留为展示兜底）
> - **多网卡 / 多 IP**：独立网卡清单，记录接口名、MAC、多 IP、掩码、VLAN、网关、bond 主从与主接口
> - **带外管理**：IPMI / iDRAC / iLO 地址
> - **云与虚拟化身份**：云实例 ID、地域 / 可用区、实例规格；虚拟机所属虚拟化平台
> - **资产类型扩展**：新增存储设备、安全设备、负载均衡三类
> - **机房机柜图**（左侧“拓扑”分组 → “机房机柜图”）：机房 → 机柜 → **自下而上 42U** 机柜立面，设备按 U 位精准落位；点击设备查看网口与同机房上联设备（交换机 / 存储 / 安全 / 负载均衡）及端口，并对越界、U 位重叠、未定位设备给出告警
>
> **数据库迁移（重要）**：本次包含两个新迁移——`h5c0d4e6f2a9`（硬件字段与网卡表）、`i6d1e5f7a3b0`（机柜实体与端口级连线）。**升级前务必备份 `cmdb.db` 与 `keystore.json`。**
>
> ```bash
> # 裸机部署：进入 backend 并激活虚拟环境后执行
> # Windows:  .venv\Scripts\activate
> # Linux:    source .venv/bin/activate
> cd backend && alembic upgrade head
> ```
>
> 加密库（V0.6.5 及以上）推荐直接**重启后在浏览器解锁**，迁移在解锁流程中自动执行；旧资产的 `(机房, 机柜)` 会自动去重回填为机柜实体。Docker 用户无需手工操作——容器启动并解锁后自动迁移。
>
> 安全加固内容（修复 **2 个高危、5 个中危**）见 [V0.7 安全加固章节](#v07--安全加固)。
>
> ---
>
> ### V0.6.5 — 数据库加密升级
...
> ```
>
> **请勿跳过此步骤。** 未迁移直接启动 V0.6.5 将进入 LOCKED 状态，无法访问原有数据。详见 [V0.6.5 章节](#v065--数据库静态加密)。
>
> ---
>
> ### V0.5.1 — 数据库迁移
...
V0.6.5 为 SQLite 数据库新增**静态加密**能力。`.db` 文件被单独窃取后无法用任何 SQLite 工具打开——必须在程序内通过管理员口令或恢复码解锁才能访问数据。

### 信封加密设计

| 层级 | 作用 |
|---|---|
| **DEK**（数据加密密钥） | 32 字节随机密钥，SQLCipher/MC 的真实库密钥。永不明文落盘，解锁后仅驻留内存 |
| **KEK**（密钥加密密钥） | 管理员口令经 Argon2id 派生（每用户独立 salt），用于包裹/解包 DEK |
| **keystore.json** | 存储 wrapped DEK + salt + nonce（全为密文）。可安全备份；无口令泄露无害 |

- KDF：Argon2id（`memory=64MB, time=3, parallelism=4`）
- 包裹算法：AES-256-GCM（每次 wrap 生成 12 字节随机 nonce）

### 应用状态机

应用以 **LOCKED** 状态启动（DB 引擎未初始化）。setup 或解锁后切换为 **UNLOCKED**（DEK 驻留内存）。

| 操作 | 效果 |
|---|---|
| **Setup**（首次运行） | 生成 DEK → 创建管理员 → 生成一次性恢复码 → 加密库 → 解锁 |
| **口令解锁** | 派生 KEK → 解包 DEK → 初始化引擎 → 运行迁移 → 签发 JWT（单口令体验） |
| **恢复码解锁** | 同上，但**不自动登录**（解锁后需用真实管理员口令登录） |
| **改密** | 用新 KEK 重新包裹同一 DEK，库本身一字节不动 |
| **增管理员** | 用新管理员 KEK 包裹 DEK → 加 keystore 记录 |
| **删管理员** | 移除 keystore 记录 → 口令立即失效 |

### 部署变化

- **启动脚本不再执行 `alembic upgrade head`**。迁移在解锁/setup 后由应用自动执行（加密库在无 DEK 时无法迁移）。
- **Windows/Linux/Docker** 三种部署均支持加密。无人值守模式通过 `CMDB_UNLOCK_PASSWORD` 环境变量注入（运行时注入，绝不写入 `.env`）。
- **一次性迁移工具**，将现有明文库加密：`python tools/encrypt_existing_db.py --password '<管理员口令>'`

### ⚠️ 重要提示

- **恢复码**：Setup 时一次性生成、一次性展示。是所有管理员口令丢失时的**唯一出路**。**务必立即离线保存**（纸质、密码管理器）。
- **备份**：必须同时备份 `cmdb.db` **和** `keystore.json`。keystore 丢失且无恢复码 = **数据永久不可读**。
- **防护范围**：单独窃取 `.db` / 窃取 `.db` + `keystore.json`（无口令）/ 整机磁盘镜像（程序未运行）均有效防护。
- **不防护范围**：运行时进程内存 dump / 主机已沦陷被键盘记录（见 PRD §1 非目标）。
...
- 支持物理服务器、虚拟机、**云服务器**、网络设备、**存储设备、安全设备、负载均衡**、其他共 8 种资产类型
- 记录硬件唯一身份：**序列号 SN、品牌厂商、型号、资产标签**，以及带外管理（IPMI/iDRAC/iLO）地址
...
| 标签页 | 内容 |
|--------|------|
| 基本信息 | 资产编号、IP/MAC/主机名、OS、硬件身份与归属、硬件规格、采购保修 |
| 端口 | 扫描发现的开放端口，含服务名、版本、状态 |
| 网络接口 | 多网卡/多 IP、VLAN、bond，并登记同机房上联设备与交换机端口 |
...
- 以机房为单位聚合机柜，管理者一眼看到**有哪些机房、各几个机柜、U 位利用率**
- 机柜按 **42U 立面（U1 在底部，自下而上）**绘制，服务器依据登记的 U 位精准落位，按设备类型配色
- 点击设备，侧栏展示其网口与**同机房上联设备（交换机 / 存储 / 安全 / 负载均衡）及端口**，所连设备同步高亮
- 数据质量告警：U 位越界、U 位重叠、未定位设备，支持快速补录
